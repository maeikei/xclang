...
const map<string,string> XClangOptions::m_supported_options =
...
void XClangOptions::parseArgs(void)
{
    try {
        ...
        desc.add_options()
            ...
            ("D",po::value<string>(),"   <macro>              Predefine the specified macro")
            ("E","                        Only run the preprocessor")
            ("Eonly","                    Just run preprocessor, no output (for timings)")
            ("F",po::value<string>(),"   <directory>          Add directory to framework include search path")
            ("H","                        Show header includes and nesting depth")
            ("I",po::value<string>(),"   <directory>          Add directory to include search path")
            ("MG","                       Add missing headers to dependency list")
            ("MP","                       Create phony target for each dependency (other than main file)")
            ("MQ",po::value<string>(),"   <value>             Specify target to quote for dependency")
            ("MT",po::value<string>(),"   <value>             Specify target for dependency")
            ("O",po::value<string>(),"                 Optimization level")
            ...
            ("U",po::value<string>(),"   <macro>              Undefine the specified macro")
            ("Wa",po::value<string>(),"                 Pass the comma separated arguments in <arg> to the assembler")
            ("Wl",po::value<string>(),"                 Pass the comma separated arguments in <arg> to the linker")
            ("Wlarge-by-value-copy",po::value<string>(),"   <value>                          Warn if a function definition returns or accepts an object larger in bytes that a given value")
            ("Wno-rewrite-macros","       Silence ObjC rewriting warnings")
            ("Wp",po::value<string>(),"                 Pass the comma separated arguments in <arg> to the preprocessor")
            ("Xanalyzer",po::value<string>(),"   <arg>        Pass <arg> to the static analyzer")
            ("Xassembler",po::value<string>(),"   <arg>       Pass <arg> to the assembler")
            ("Xclang",po::value<string>(),"   <arg>           Pass <arg> to the clang compiler")
            ("Xlinker",po::value<string>(),"   <arg>          Pass <arg> to the linker")
            ("Xpreprocessor",po::value<string>(),"   <arg>    Pass <arg> to the preprocessor")
            ("add-plugin",po::value<string>(),"   <name>      Use the named plugin action in addition to the default action")
            ("analyze","                 Run the static analyzer")
            ("analyze-function",po::value<string>(),"   <value>                          Run analysis on specific function")
            ("analyzer-checker",po::value<string>(),"   <value>                          Choose analyzer checkers to enable")
            ("analyzer-checker-help","    Display the list of analyzer checkers that are available")
            ("analyzer-constraints",po::value<string>(),"   <value>                          Source Code Analysis - Symbolic Constraint Engines")
            ("analyzer-disable-checker",po::value<string>(),"   <value>                          Choose analyzer checkers to disable")
            ...
            ("analyzer-inline-max-function-size",po::value<string>(),"   <value>                          Bound on the number of basic blocks in an inlined function (200 by default)")
            ("analyzer-inline-max-stack-depth",po::value<string>(),"   <value>                          Bound on stack depth while inlining (4 by default)")
            ("analyzer-inlining-mode",po::value<string>(),"   <value>                          Specify the function selection heuristic used during inlining")
            ("analyzer-ipa",po::value<string>(),"   <value>   Specify the inter-procedural analysis mode")
            ("analyzer-max-loop",po::value<string>(),"   <value>                          The maximum number of times the analyzer will go through a loop")
            ("analyzer-max-nodes",po::value<string>(),"   <value>                          The maximum number of nodes the analyzer can generate (150000 default, 0 = no limit)")
            ...
            ("analyzer-output",po::value<string>(),"   <value>                          Source Code Analysis - Output Options")
            ("analyzer-purge",po::value<string>(),"   <value> Source Code Analysis - Dead Symbol Removal Frequency")
            ("analyzer-stats","           Print internal analyzer statistics.")
            ("analyzer-store",po::value<string>(),"   <value> Source Code Analysis - Abstract Memory Store Models")
            ...
            ("arcmt-migrate-report-output",po::value<string>(),"   <value>                          Output path for the plist report")
            ...
            ("backend-option",po::value<string>(),"   <value> Additional arguments to forward to LLVM backend (during code gen)")
            ("c","                        Only run preprocess, compile, and assemble steps")
            ("c-isystem",po::value<string>(),"   <directory>  Add directory to the C SYSTEM include search path")
            ...
            ("chain-include",po::value<string>(),"   <file>   Include and chain a header file after turning it into PCH")
            ...
            ("cl-std",po::value<string>(),"           OpenCL language standard to compile for")
            ...
            ("code-completion-at",po::value<string>(),"   <file>:<line>:<column>                          Dump code-completion information at a location")
            ...
            ("coverage-file",po::value<string>(),"   <value>  Emit coverage data to this filename. The extension will be replaced.")
            ("cxx-abi",po::value<string>(),"   <value>        Target a particular C++ ABI type")
            ("cxx-isystem",po::value<string>(),"   <directory>                          Add directory to the C++ SYSTEM include search path")
            ("dD","                       Print macro definitions in -E mode in addition to normal output")
            ("dM","                       Print macro definitions in -E mode instead of normal output")
            ("dependency-dot",po::value<string>(),"   <value> Filename to write DOT-formatted header dependencies to")
            ("dependency-file",po::value<string>(),"   <value>                          Filename (or -) to write dependency output to")
            ("detailed-preprocessing-record","                            include a detailed record of preprocessing actions")
            ("diagnostic-log-file",po::value<string>(),"   <value>                          Filename (or -) to log diagnostics to")
            ...
            ("dump-build-information",po::value<string>(),"   <filename>                          output a dump of some build information to a file")
            ...
            ("dwarf-debug-flags",po::value<string>(),"   <value>                          The string to embed in the Dwarf debug flags record.")
            ...
            ("error-on-deserialized-decl",po::value<string>(),"   <value>                          Emit error if a specific declaration is deserialized from PCH, for testing")
            ...
            ("fconstexpr-backtrace-limit",po::value<string>(),"   <N>                          Set the maximum number of entries to print in a constexpr evaluation backtrace (0 = no limit).")
            ("fconstexpr-depth",po::value<string>(),"   <value>                          Maximum depth of recursive constexpr function calls")
            ("fcuda-is-device","          Generate code for CUDA device")
            ("fcxx-exceptions","          Enable C++ exceptions")
            ("fdata-sections","           Place each data in its own section (ELF Only)")
            ("fdebug-compilation-dir",po::value<string>(),"   <value>                          The compilation directory to embed in the debug info.")
            ...
            ("fdiagnostics-format",po::value<string>(),"   <value>                          Change diagnostic formatting to match IDE and command line tools")
            ...
            ("fdiagnostics-show-category",po::value<string>(),"   <value>                          Print diagnostic category")
            ...
            ("ferror-limit",po::value<string>(),"   <N>       Set the maximum number of errors to emit before stopping (0 = no limit).")
            ...
            ("fixit",po::value<string>(),"            Apply fix-it advice creating a file with the given suffix")
            ...
            ("fmacro-backtrace-limit",po::value<string>(),"   <N>                          Set the maximum number of entries to print in a macro expansion backtrace (0 = no limit).")
            ("fmath-errno","              Require math functions to indicate errors by setting errno")
            ("fmessage-length",po::value<string>(),"   <N>    Format message diagnostics so that they fit within N columns or fewer, when possible.")
            ("fmodule-cache-path",po::value<string>(),"   <directory>                          Specify the module cache path")
            ("fmodule-name",po::value<string>(),"      Specify the name of the module to build")
            ("fmodules","                 Enable the 'modules' language feature")
            ("fms-compatibility","        Enable Microsoft compatibility mode")
            ("fms-extensions","           Accept some non-standard constructs supported by the Microsoft compiler")
            ("fmsc-version",po::value<string>(),"     Version of the Microsoft C/C++ compiler to report in _MSC_VER (0 = don't define it (default))")
            ...
            ("fobjc-arc-cxxlib",po::value<string>(),"                            Objective-C++ Automatic Reference Counting standard library kind")
            ("fobjc-arc-exceptions","     Use EH-safe code when synthesizing retains and releases in -fobjc-arc")
            ("fobjc-default-synthesize-properties","                            enable the default synthesis of Objective-C properties")
            ("fobjc-dispatch-method",po::value<string>(),"                            Objective-C dispatch method to use")
            ...
            ("foverride-record-layout",po::value<string>(),"                            Override record layouts with those in the given file")
            ("fpack-struct",po::value<string>(),"   <value>   Specify the default maximum struct packing alignment")
            ("fpascal-strings","          Recognize and construct Pascal-style string literals")
            ("fshort-enums","             Allocate to an enum type only as many bytes as it needs for the declared range of possible values")
            ("fshort-wchar","             Force wchar_t to be a short unsigned int")
            ("fshow-column","             Include column number on diagnostics")
            ("fshow-overloads",po::value<string>(),"                            Which overload candidates to show when overload resolution fails: best|all; defaults to all")
            ("fsjlj-exceptions","         Use SjLj style exceptions")
            ("fsyntax-only","             Run parser and perform semantic analysis")
            ("ftabstop",po::value<string>(),"   <N>           Set the tab stop distance.")
            ("ftemplate-backtrace-limit",po::value<string>(),"   <N>                          Set the maximum number of entries to print in a template instantiation backtrace (0 = no limit).")
            ("ftemplate-depth",po::value<string>(),"   <value>                          Maximum depth of recursive template instantiation")
            ("ftime-report","             Print the amount of time each phase of compilation takes")
            ("ftrap-function",po::value<string>(),"   Issue call to specified function rather than a trap instruction")
            ...
            ("fvisibility",po::value<string>(),"   <value>    Default symbol visibility")
            ("fvisibility-inlines-hidden","                            Give inline C++ member functions default visibility by default")
            ("fwrapv","                   Treat signed integer overflow as two's complement")
            ("fwritable-strings","        Store string literals as writable data")
            ("g","                        Generate source level debug information")
            ("gcc-toolchain",po::value<string>(),"   <value>  Use the gcc toolchain at the given directory")
            ("header-include-file",po::value<string>(),"   <value>                          Filename (or -) to write header include output to")
            ("help","                    Display available options")
            ("idirafter",po::value<string>(),"   <directory>  Add directory to AFTER include search path")
            ("iframework",po::value<string>(),"   <directory> Add directory to SYSTEM framework search path")
            ("imacros",po::value<string>(),"   <file>         Include macros from file before parsing")
            ("include",po::value<string>(),"   <file>         Include file before parsing")
            ("include-pch",po::value<string>(),"   <file>     Include precompiled header file")
            ("include-pth",po::value<string>(),"   <file>     Include file before parsing")
            ("index-header-map","         Make the next included directory (-I or -F) an indexer header map")
            ("init-only","                Only execute frontend initialization")
            ("internal-externc-isystem",po::value<string>(),"   <directory>                          Add directory to the internal system include search path with implicit extern \"C\" semantics; these are assumed to not be user-provided and are used to model system and standard headers' paths.")
            ("internal-isystem",po::value<string>(),"   <directory>                          Add directory to the internal system include search path; these are assumed to not be user-provided and are used to model system and standard headers' paths.")
            ("iprefix",po::value<string>(),"   <prefix>       Set the -iwithprefix/-iwithprefixbefore prefix")
            ("iquote",po::value<string>(),"   <directory>     Add directory to QUOTE include search path")
            ("isysroot",po::value<string>(),"   <dir>         Set the system root directory (usually /)")
            ("isystem",po::value<string>(),"   <directory>    Add directory to SYSTEM include search path")
            ("iwithprefix",po::value<string>(),"   <dir>      Set directory to SYSTEM include search path with prefix")
            ("iwithprefixbefore",po::value<string>(),"   <dir>                          Set directory to include search path with prefix")
            ("iwithsysroot",po::value<string>(),"   <directory>                          Add directory to SYSTEM include search path, absolute paths are relative to -isysroot")
            ("load",po::value<string>(),"   <dsopath>         Load the named plugin (dynamic shared object)")
            ("main-file-name",po::value<string>(),"   <value> Main file name to use for debug info")
            ("masm-verbose","             Generate verbose assembly output")
            ("mcode-model",po::value<string>(),"   <value>    The code model to use")
            ("mconstructor-aliases","     Emit complete constructors and destructors as aliases when possible")
            ("mdebug-pass",po::value<string>(),"   <value>    Enable additional debug output")
            ...
            ("mfloat-abi",po::value<string>(),"   <value>     The float ABI to use")
            ("migrate","                 Run the migrator")
            ("mlimit-float-precision",po::value<string>(),"   <value>                          Limit float precision to the given value")
            ("mlink-bitcode-file",po::value<string>(),"   <value>                          Link the given bitcode file before performing optimizations.")
            ("mllvm",po::value<string>(),"   <value>          Additional arguments to forward to LLVM's option processing")
            ...
            ("mregparm",po::value<string>(),"   <value>       Limit the number of registers available for integer arguments")
            ("mrelax-all","               (integrated-as) Relax all machine instructions")
            ("mrelocation-model",po::value<string>(),"   <value>                          The relocation model to use")
            ("mrtd","                     Make StdCall calling convention the default")
            ("msave-temp-labels","        (integrated-as) Save temporary labels")
            ("msoft-float","              Use software floating point")
            ("mstack-alignment",po::value<string>(),"                            Set the stack alignment")
            ("mstackrealign","            Force realign the stack at entry to every function.")
            ("mt-migrate-directory",po::value<string>(),"   <value>                          Directory for temporary files produced during ARC or ObjC migration")
            ...
            ("o",po::value<string>(),"   <file>               Write output to <file>")
            ("objc-isystem",po::value<string>(),"   <directory>                          Add directory to the ObjC SYSTEM include search path")
            ...
            ("objcxx-isystem",po::value<string>(),"   <directory>                          Add directory to the ObjC++ SYSTEM include search path")
            ("pg","                       Enable mcount instrumentation")
            ("pic-level",po::value<string>(),"   <value>      Value for __PIC__")
            ("pipe","                     Use pipes between commands, when possible")
            ("plugin",po::value<string>(),"   <name>          Use the named plugin action instead of the default action (use \"help\" to list available options)")
            ("plugin-arg",po::value<string>(),"   <arg>                          Pass <arg> to plugin <name>")
            ("preamble-bytes",po::value<string>(),"   Assume that the precompiled header is a precompiled preamble covering the first N bytes of the main file")
            ("print-decl-contexts","      Print DeclContexts and their Decls")
            ("print-file-name",po::value<string>(),"   Print the full library path of <file>")
            ...
            ("print-prog-name",po::value<string>(),"   Print the full program path of <name>")
            ...
            ("remap-file",po::value<string>(),"   <from>;<to> Replace the contents of the <from> file with the contents of the <to> file")
            ("resource-dir",po::value<string>(),"   <value>   The directory which holds the compiler resource files")
            ("rewrite-macros","           Expand macros without full preprocessing")
            ("rewrite-objc","             Rewrite Objective-C source to C++")
            ("rewrite-test","             Rewriter playground")
            ("save-temps","               Save intermediate compilation results")
            ("serialize-diagnostic-file",po::value<string>(),"   <filename>                          File for serializing diagnostics in a binary format")
            ("serialize-diagnostics",po::value<string>(),"   <value>                          Serialize compiler diagnostics to a file")
            ("stack-protector",po::value<string>(),"   <value>                          Enable stack protectors")
            ("static-define","            Should __STATIC__ be defined")
            ("std",po::value<string>(),"              Language standard to compile for")
            ("stdlib",po::value<string>(),"           C++ standard library to use")
            ("sys-header-deps","          Include system headers in dependency output")
            ("target",po::value<string>(),"   <value>         Generate code for the given target")
            ("target-abi",po::value<string>(),"   <value>     Target a particular ABI type")
            ("target-cpu",po::value<string>(),"   <value>     Target a specific cpu type")
            ("target-feature",po::value<string>(),"   <value> Target specific attributes")
            ("target-linker-version",po::value<string>(),"   <value>                          Target linker version")
            ("time","                     Time individual commands")
            ("token-cache",po::value<string>(),"   <path>     Use specified token cache file")
            ("traditional-cpp","          Enable some traditional CPP emulation")
            ("trigraphs","                Process trigraph sequences")
            ("trim-egraph","              Only show error-related paths in the analysis graph")
            ("triple",po::value<string>(),"   <value>         Specify target triple (e.g. i686-apple-darwin9)")
            ...
            ("working-directory",po::value<string>(),"   <value>                          Resolve file paths relative to the specified directory")
        ;
        po::variables_map vm;
